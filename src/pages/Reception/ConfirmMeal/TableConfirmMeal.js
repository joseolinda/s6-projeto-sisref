...
const TableConfirmMeal = props =>{
    ...
    }, [formState.values, student, mealSearch]);
    ...
    async function loadStudentsconfirmMeal(page){
        const date = dateSearch;
        if(!date){
            date = handleData();
        }
        try {
            let url = 'confirm-meals/list?page='+page+'&date='+date;
    ...
    async function loadStudent(){
        ...
        if(!mat){
            loadAlert('error', 'Informe a matrícula ou o código.');
            return;
        }
    ...
    async function onConfirmObject(){
        try {
          ...
          if (response.status === 202) {
            ...
            loadAlert('success', 'Refeição confirmada.');
            loadStudentsconfirmMeal(page+1);
            setFormState(formState => ({
                ...formState,
                values: {
                    ...formState.values,
    ...
    return (
        <div>
            ...
            <div className={classes.row}>
                <Typography variant="h5" className={classes.title}>{'Confirmação das Refeições'}</Typography>
            </div>
            <br></br>
            <Grid
                container
...
TableConfirmMeal.propTypes = {
    history: PropTypes.object
  };

export default TableConfirmMeal;
