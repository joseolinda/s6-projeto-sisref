...
const ShiftDetails = props => {
  ...
  const { idShift } = props.match.params;
  ...
  async function saveShiftDetails(){
    try {
      ...
      let acao = "";
      if(!idShift) {
        response = await api.post('shift', data);
        acao = "cadastrado";
      } else {
        response = await api.put('shift/'+idShift, data);
        acao = "atualizado";
      }
      if (response.status === 200) {
        loadAlert('success', 'Turno '+acao+'.');
        history.push('/shift');
      } else {
  ...
  async function findAShift(){
    try {
      const response = await api.get('shift/show/'+idShift);
      if (response.status === 200) {
        if(response.data.message){
          loadAlert('error', response.data.message);
        }
